...
if($cek_jabatan == "admin" AND $identitas_jabatan == "admin"){
    ...
    include 'header.php';
        ...
        if (isset($_POST['save'])) {
            $post_perangkat = $_POST['perangkat'];
            $post_type = $_POST['type'];

            $check_device = mysqli_query($conn, "SELECT * FROM tb_device WHERE perangkat = '$post_perangkat'");
            $data_device = mysqli_fetch_assoc($check_device);

            if(empty($post_perangkat) || empty($post_type)) {
                $msg_type = "error";
                ...
                $msg_type = "error";
                $msg_content = '<b>Gagal:</b> Device sudah terdaftar sebelumnya.<script>swal("Gagal!", "Device sudah terdaftar sebelumnya!", "error");</script>';
            } else {
                $insert = mysqli_query($conn, "INSERT INTO tb_device VALUES('','$post_perangkat','$post_type')");
                if($insert == TRUE){
                    $msg_type = "success";
                            ...
                            <div class="accordion-item">
                                <h2 class="accordion-header" id="flush-headingOne">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseOne" aria-expanded="false" aria-controls="flush-collapseOne">
                                        <strong class="card-title">Tambah Data Device</strong>
                                    </button>
                                </h2>
                                            ...
                                            <div class="row form-group">
                                                ...
                                                <div class="col-12 col-lg-10">
                                                    <p class="form-control-static">
                                                        <input type="text" id="text-input" name="perangkat" placeholder="Masukkan nama perangkat" class="form-control" autocomplete="off" required>
                                                    </p>
                                                </div>
                                            </div>
                                            <div class="row form-group">
                                                ...
                                                <div class="col-12 col-lg-10">
                                                    <p class="form-control-static">
                                                        <input type="text" id="text-input" name="type" placeholder="Masukkan type perangkat" class="form-control" autocomplete="off" required>
                                                    </p>
                                                </div>
                                            </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body table-responsive">
                            <table id="bootstrap-data-table-export" class="table table-striped table-bordered">
                                ...
                                <tbody>
                                    <?php
                                        $check_data_master = mysqli_query($conn, "SELECT * FROM tb_device ORDER BY id DESC");
                                        $no = 1;
                                        while($data_master = mysqli_fetch_assoc($check_data_master)) {
                                    ?>
                                    <tr>
                                        <td><?php echo $no++; ?></td>
                                        <td><?php echo $data_master['perangkat']; ?></td>
                                        <td><?php echo $data_master['type']; ?></td>
                                        <td class="text-center"><a href="<?php echo $base_link; ?>edit-data-device.php?data=<?php echo $data_master['perangkat']; ?>" class="btn btn-warning btn-sm"><i class="fa fa-edit"></i></a> <a href="delete-data-device.php?data=<?php echo $data_master['perangkat']; ?>" class="btn btn-sm btn-danger alert_notif"><i class="fa fa-trash"></i></a></td>
                                    </tr>
                                    <?php
                                        }
                                    ?>
